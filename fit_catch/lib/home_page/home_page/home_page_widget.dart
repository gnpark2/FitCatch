...
class _HomePageWidgetState extends State<HomePageWidget> {
  ...
  // 함수 추가: 스타일별 옷 추천 로직
  Map<String, ClothRecord?> getRandomCoordination(List<ClothRecord> clothes) {
    ...
    return {
      'top': coordination[0],
      'bottom': coordination[1],
      'outer': coordination[2],
      'accessory': coordination[3],
    };
  }

  // 함수 추가: 코디 저장 함수
  Future<void> toggleSaveCoordination(Map<String, ClothRecord?> coordination) async {
    if (isCoordinationSaved) {
      // 코디 삭제
      final querySnapshot = await FirebaseFirestore.instance
          .collection('coordination')
          .where('user', isEqualTo: currentUserReference)
          .where('top', isEqualTo: coordination['top']?.reference)
          .where('bottom', isEqualTo: coordination['bottom']?.reference)
          .where('outer', isEqualTo: coordination['outer']?.reference)
          .where('accessory', isEqualTo: coordination['accessory']?.reference)
          .get();
      ...
      final coordinationRecordData = createCoordinationRecordData(
        ...
        top: coordination['top']?.reference,
        bottom: coordination['bottom']?.reference,
        outer: coordination['outer']?.reference,
        accessory: coordination['accessory']?.reference,
      );

      await CoordinationRecord.collection.doc().set(coordinationRecordData);
    }
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<ClothRecord>>(
      ...
      builder: (context, snapshot) {
        ...
        var coordination = getRandomCoordination(homePageClothRecordList);
                                          ...
                                          child: Container(
                                            ...
                                            child: Stack(
                                              ...
                                              children: [
                                                ...
                                                if (coordination['accessory'] != null)
                                                  Align(
                                                    alignment:
                                                        AlignmentDirectional(
                                                            0.9, 0.9),
                                                  ...
                                                  child: IconButton(
                                                    ...
                                                    onPressed: () async {
                                                      await toggleSaveCoordination(coordination);
...
class _HomePageWidgetState extends State<HomePageWidget> {
  ...
  Future<void> _checkIfCoordinationSaved() async {
    final querySnapshot = await FirebaseFirestore.instance
        .collection('coordination')
        .where('user', isEqualTo: currentUserReference)
        .where('top', isEqualTo: todayCoordination!['top'])
        .where('bottom', isEqualTo: todayCoordination!['bottom'])
        .where('outer', isEqualTo: todayCoordination!['outer'])
        .where('accessory', isEqualTo: todayCoordination!['accessory'])
        .get();
